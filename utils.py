...
# In your tests, it took over 16 seconds versus less than 4 seconds for the implementation below (where X has 69,000 elements and Y had 300)
...
def pdist2(X, Y, metric):
  ...
  if metric == 'sqeuclidean':
    ...
    XY = X.dot(Y.T)
    sqeuc = XX + YY - 2*XY
    # make negatives equal to zero. This arises due to floating point precision issues.
    # Negatives will be very close to zero (IIRC around -1e-10 or maybe even closer to zero). Any better fix?
    # you exhibited the floating point issue on two machines using the same code and data, but not on a third.
    # the inconsistent occurrence of the issue could possibly be due to differences in numpy/blas versions across machines. 
    return np.clip(sqeuc, 0, np.inf)
  elif metric == 'hamming':
    # scipy cdist supports hamming distance, but is twice as slow as yours (even before multiplying
    # by dim, and casting as int), possibly because it supports non-booleans, but I'm not sure...
    #####
    # looping over data points in X and Y, and calculating hamming distance
    # to put in a hamdis matrix is too slow. this vectorized solution works faster.
    # separately, the matlab solution that uses compactbit is approximately 8x more
    # memory efficient, since 8 bits are required here for each 0 or 1.
    hashbits = X.shape[1]
...
# Assumes columns contain variables/features, and rows contain observations/instances
def standardize(X):
